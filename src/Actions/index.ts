...
export default class Main {
  ...
  setUserTimesheetRow() {
    ...
    let resultAPI: any = api.getUsersTimesheetRow(
      JSON.parse(this._properties.projectrow.toString())
    );
  ...
  cleanTable() {
    ...
    let firstRowDev = spreadsheet.suiviSheet
      .createTextFinder("CONSOMMÉ EN JOURS DEV")
      .findNext()
      .getRow();

    let lastRowDev = spreadsheet.suiviSheet
      .getRange(firstRowDev, 1, 100, 1)
    ...
    let rangeDev = spreadsheet.suiviSheet.getRange(
      firstRowDev + 3,
      2,
      lastRowDev - firstRowDev - 3,
      lastColData - 2
    );
    rangeDev.setValue(null);

    let firstRowUX = spreadsheet.suiviSheet
      .createTextFinder("CONSOMMÉ EN JOURS UX/UI")
      .findNext()
      .getRow();

    let lastRowUX = spreadsheet.suiviSheet
      .getRange(firstRowUX, 1, 100, 1)
      .createTextFinder("TOTAL")
      .findNext()
      .getRow();

    console.log("lastRowUX", lastRowUX);

    let rangeUX = spreadsheet.suiviSheet.getRange(
      firstRowUX + 3,
      2,
      lastRowUX - firstRowUX - 4,
      lastColData - 2
    );
    rangeUX.setValue(null);
  }
}
